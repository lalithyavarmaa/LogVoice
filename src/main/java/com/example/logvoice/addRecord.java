        ...
        if (recordingId != null) {
            reference.child(recordingId).setValue(recordingData).addOnCompleteListener(task -> {
                if (task.isSuccessful()) {
                    Toast.makeText(addRecord.this, "Recording data saved to Firebase", Toast.LENGTH_SHORT).show();
                    navigateToRecordingsActivity();
                } else {
                    Toast.makeText(addRecord.this, "Failed to save recording to Firebase", Toast.LENGTH_SHORT).show();
